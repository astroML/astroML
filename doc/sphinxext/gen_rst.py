"""
Example generation developed for scikit-learn
...
from gen_figure_rst import create_thumbnail
...
[download: :download:`%(fname)s <%(fname)s>`]
...
""" % file(os.path.join(src_dir, 'README.txt')).read())
    ...
    for fname in sorted(os.listdir(src_dir), key=sort_key):
        if fname.endswith('py'):
            ...
            thumb = os.path.join(dir, 'images', 'thumb',
                                 fname[:-3] + '_thumb.png')
...
def generate_file_rst(fname, target_dir, src_dir, plot_gallery):
    ...
    if plot_gallery and fname.startswith('plot'):
        ...
        if os.path.exists(first_image_file):
            create_thumbnail(first_image_file, thumb_file, 0.2)
    ...
    if len(figure_list) == 1:
        ...
        image_list = SINGLE_IMAGE_W100 % figure_name.lstrip('/')
    else:
...
def setup(app):
    ...
    # remove images in the build dir: sphinx does not overwrite them
    # by default.
